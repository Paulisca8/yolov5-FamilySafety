...
def run(
    ...
    cap = cv2.VideoCapture("data/video/video7.mp4")
    frame_id = 0
    # 设置视频输出参数
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    ok, im = cap.read()
    out = cv2.VideoWriter('output_video.mp4', fourcc, 30.0, (im.shape[1], im.shape[0]))
        ...
        for i, det in enumerate(pred):  # per image
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    cv2.rectangle(im0, p1, p2, (56, 56, 255) if cls == 0 else (151, 157, 255), 2)
                    cv2.putText(im0, label, (p1[0], p1[1] - 2), cv2.FONT_HERSHEY_PLAIN, 2, (56, 56, 255) if cls == 0 else (151, 157, 255), 2)
        # 将处理后的帧写入视频
        out.write(im0)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'models/best6.pt', help='model path(s)')
